...
- (void)viewDidLoad {
    ...
    searchController.searchBar.placeholder = @"软件包";
...
- (void)configureClearSearchButton {
    self.navigationItem.rightBarButtonItem = recentSearches.count ? [[UIBarButtonItem alloc] initWithTitle:@"清除搜索历史" style:UIBarButtonItemStylePlain target:self action:@selector(clearSearches)] : nil;
}
...
- (void)searchBarSearchButtonClicked:(UISearchBar *)searchBar {
    ...
    if (query.length <= 1) {
        [ZBAppDelegate sendErrorToTabController:@"😠咱们能不能搜个完整的名字,你这么短有啥用,去搜个完整的去！😠"];
        return;
    }
