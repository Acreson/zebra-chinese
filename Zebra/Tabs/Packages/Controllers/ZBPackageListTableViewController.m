...
- (void)configureUpgradeButton {
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self->needsUpdatesSection) {
            UIBarButtonItem *updateButton = [[UIBarButtonItem alloc] initWithTitle:@"全部更新" style:UIBarButtonItemStylePlain target:self action:@selector(upgradeAll)];
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self.continueBatchLoad) {
            if (self->totalNumberOfPackages) {
                UIBarButtonItem *loadButton = [[UIBarButtonItem alloc] initWithTitle:[NSString stringWithFormat:@"%.0f%% 加载完成", MIN(100, (double)(self->numberOfPackages * 100) / self->totalNumberOfPackages)] style:UIBarButtonItemStylePlain target:self action:@selector(loadNextPackages)];
...
- (void)configureSegmentedController {
    ...
    UISegmentedControl *segmentedControl = [[UISegmentedControl alloc] initWithItems:@[@"名称", @"日期"]];
...
- (void)configureQueueOrShareButton {
    dispatch_async(dispatch_get_main_queue(), ^{
        if ([[ZBQueue sharedInstance] hasObjects]) {
            self->queueButton = [[UIBarButtonItem alloc] initWithTitle:@"队列" style:UIBarButtonItemStylePlain target:self action:@selector(presentQueue)];
            self->clearButton = [[UIBarButtonItem alloc] initWithTitle:@"清除" style:UIBarButtonItemStylePlain target:self action:@selector(askClearQueue)];
